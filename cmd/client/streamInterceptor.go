...
func (s *myClientStreamWrapper1) SendMsg(m any) error {
	log.Println("[pre message] my stram client interceptor 1: ", m)
...
func (s *myClientStreamWrapper2) SendMsg(m any) error {
	log.Println("[pre message] my stram client interceptor 2: ", m)

	return s.ClientStream.SendMsg(m)
}

func (s *myClientStreamWrapper2) RecvMsg(m any) error {
	err := s.ClientStream.RecvMsg(m)
